/*
	...
	Modified:
	Broadcasts _varName via remoteExec. Hides object on creation.
*/
...

{
	...
	if ((random 1) > _rdm) then 
	{
		...
		//Hide
		[_newObj, true] remoteExec ["hideObjectGlobal", 2];
		...
		if (!isNil "_varName") then 
		{
			if (_varName != "") then 
			{
				...
				missionNamespace setVariable [_varName, _newObj, true];
				[_newObj, _varName] remoteExec ["setVehicleVarName", 0, true];
				publicVariable (str (missionNamespace getVariable _varName));
